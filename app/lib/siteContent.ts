...
export function getMenuFooterLine(site?: Site | null): string {
  ...
  const businessDesc = tiptapToText(site?.business?.description);
  if (businessDesc) {
    return businessDesc.length > 140 ? `${businessDesc.slice(0, 140).trim()}…` : businessDesc;
  }

  if (typeof site?.footer?.description === 'string' && site.footer.description.trim()) {
    return site.footer.description.trim();
  }
  return tiptapToText(site?.footer?.description);
}
